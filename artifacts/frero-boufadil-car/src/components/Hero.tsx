...
export default function Hero() {
  ...
  const titleComponent = (
    <div className="flex flex-col items-center gap-6 pt-24 md:pt-0">
      ...
      <motion.p
        className="text-[var(--c-text-muted)] text-base sm:text-lg max-w-xl mx-auto"
      ...
      >
        {STATS.map((stat) => (
          <div key={stat.key} className="text-center">
            ...
            <div className="text-[var(--c-text-muted)] text-xs mt-0.5">{t(stat.key)}</div>
          </div>
        ))}
      </motion.div>
    </div>
  ...
  return (
    <div className="relative bg-[var(--c-bg-page)] overflow-hidden transition-colors duration-300">
